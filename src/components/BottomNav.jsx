...
// Only show nav on these exact top-level paths
const showOn = new Set(["/", "/trips", "/account"]);

export default function BottomNav() {
  const { pathname } = useLocation();
  if (!showOn.has(pathname)) return null;
  ...
  return (
    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: C.white, borderTop: `1px solid ${C.div}`, display: "flex", paddingTop: 6, paddingBottom: 26, zIndex: 20 }}>
      {tabs.map(tab => {
        ...
        return (
          <NavLink key={tab.to} to={tab.to} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, textDecoration: "none", padding: "4px 0", position: "relative" }}>
